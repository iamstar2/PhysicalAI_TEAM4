...
### 4. 대시보드 "방문 횟수" 표시용 DB 조회 API (2026-09-14 추가)
- 대시보드(백경률)에 방문 횟수를 표시하려면 최현수님의 DB(`postgres`/`api-server`)
  조회 API가 필요한데, 이 저장소에는 `access_decisions`(또는 그에 준하는) 테이블이나
  REST 엔드포인트가 아직 정의돼 있지 않음(schema/, origin/main 포함 전체 검색 결과 없음).
- **왜 단순 `COUNT(*)`가 위험한가**: `mechdog_project_0902.md`의 MQTT→DB 브릿지는
  "수신 메시지를 그대로 테이블에 적재"하는 방식이라, 한 방문자가 한 번 방문해도
  `vision.face`가 여러 번(항목 3의 재시도), `vision.ppe`가 여러 번(예:
  안전모 미착용 → 재착용 재검사) 각각 별도 행으로 쌓일 수 있음. 이 테이블을 그대로
  `COUNT(*)` 하면 재시도·재검사 횟수까지 방문 횟수로 잘못 셈.
- **제안**: `schema/README.md` §3에 명시된 대로 `session_id`는 "A가 발급하고
  B·C·D가 승계"하는, 방문자 1명의 게이트 진입~종료를 잇는 유일 키이므로
  `COUNT(DISTINCT session_id)`로 세는 것을 제안. 다만 이것도 "방문 횟수"의 정의에
  따라 갈림 — 미인가로 거부된 방문(=A→B 인계인 `gate.session`이 발생하지 않는 경우)도
  포함할지, `gate.session`이 발생한(=통과한) 경우만 셀지는 비즈니스 정의가 필요함.
- **담당자**: 최현수(DB/API) 확인 필요, 백경률(대시보드 소비자)
- **확인할 것**:
  1. `access_decisions`(가칭) 테이블의 실제 grain(행 1개 = 메시지 1건인지, 방문 1건인지)과 컬럼(특히 `session_id` 보유 여부).
  2. "방문 횟수"에 미인가/거부 시도도 포함하는지, 통과한 방문만 세는지.
  3. 조회 API 엔드포인트·요청 파라미터(기간 필터 등)·응답 스키마 (`mechdog_project_0902.md`에 FastAPI로 REST 제공 예정이라고 되어 있음).
- API/스키마가 정해지기 전까지 대시보드에는 임의의 DB 연결이나 가짜 숫자를 넣지 않음(2026-09-14 기준 미구현).
...
- [ ] **최현수**: 대시보드 "방문 횟수" 표시용 DB 조회 API 계약 확정 (항목 4 참고)
- [ ] **백경률 / 여도훈**: alert.event의 unauthorized/no_helmet/no_vest/face_timeout을
      A가 직접 발행하게 되면(현재는 D가 vision.face/ppe를 보고 대신 발행 중), A/D
      중복 발행이 되지 않도록 역할 재정리
